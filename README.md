...
```text
1、Ribbon本地负载均衡客户端
    原理：在调用接口的时候，会在注册中心获取注册服务列表，获取到后缓存到JVM本地，使用本地实现RPC调用技术调用服务，客户端实现负载均衡
    场景：在微服务RPC远程调用
2、Nginx服务器负载均衡
    原理：客户端所有请求由Nginx统一转发，由服务器端实现
    场景：针对服务器端容器调用
...
# Feign
```text
1、SpringCloud支持两种客户端调用方式:
    1.1、REST: RestTemplate(基本使用不到)
    1.2、Feign: 声明式Http客户端调用工具，采用接口+注解方式实现
